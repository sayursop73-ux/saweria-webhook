...
app.post('/', async (req, res) => {
    ...
    if (!ROBLOX_API_KEY || !UNIVERSE_ID) {
        console.error('Error: Kredensial Roblox tidak diatur!');
    ...
    try {
        ...
        console.log('Berhasil mengirim data ke Roblox.');
        res.status(200).send('OK');
    } catch (error) {
        console.error('Gagal mengirim data ke Roblox:', error.response?.data || error.message);
        res.status(500).send('Gagal meneruskan pesan.');
    }
});


app.get('/test', (req, res) => {
    res.send('Server Webhook Saweria-Roblox di Vercel berjalan!');
});
